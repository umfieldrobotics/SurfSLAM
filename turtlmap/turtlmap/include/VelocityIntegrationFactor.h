...
/**
 * @brief Saved state for PreintegratedVelocityMeasurementsDvlOnly
 *
 * Contains all internal state variables needed to restore the preintegrated
 * velocity measurements to a previous state. Useful for keyframe management.
 */
struct PreintegratedVelocityState
{
  double deltaTij;
  gtsam::Pose3 accumulated_poses;
  gtsam::Point3 accumulated_positions;
  gtsam::imuBias::ConstantBias bias;
  gtsam::Matrix3 delpdelBiasOmega;
  gtsam::Matrix3 delpdelBiasDvl;
  Eigen::Matrix<double, 3, 3> preintMeasCov;
  std::vector<gtsam::Vector3> vel_list;
  std::vector<gtsam::Rot3> rot_list;
  std::vector<double> dt_list;

  PreintegratedVelocityState()
      : deltaTij(0.0),
        accumulated_poses(gtsam::Pose3()),
        accumulated_positions(gtsam::Point3::Zero()),
        bias(gtsam::imuBias::ConstantBias()),
        delpdelBiasOmega(gtsam::Matrix3::Zero()),
        delpdelBiasDvl(gtsam::Matrix3::Zero()),
        preintMeasCov(gtsam::Matrix3::Zero())
  {}
};
...
{
  gtsam::Matrix3
      biasVelCovariance_;   ///< continuous-time "Covariance" describing velocity "measurement" bias random walk
...
class PreintegratedVelocityMeasurementsDvlOnly
{
 ...
 public:
  ...
  void integrateMeasurements(const gtsam::Vector3& linear_velocity, const gtsam::Rot3& interpolated_rot, double& dt);
   ...
   *
   * @return Current state of the preintegrated measurements
   */
  PreintegratedVelocityState saveState() const;
   ...
   */
  void loadState(const PreintegratedVelocityState& state);
};
...
class DvlOnlyFactor : public gtsam::NoiseModelFactorN<gtsam::Pose3, gtsam::Pose3, gtsam::imuBias::ConstantBias>
{
 ...
 public:
  DvlOnlyFactor() {}
  DvlOnlyFactor(gtsam::Key pose_i, gtsam::Key pose_j, gtsam::Key vbias_i, PreintegratedVelocityMeasurementsDvlOnly& pvm)
      : gtsam::NoiseModelFactorN<gtsam::Pose3, gtsam::Pose3, gtsam::imuBias::ConstantBias>(
            gtsam::noiseModel::Gaussian::Covariance(pvm.preintMeasCov_), pose_i, pose_j, vbias_i),
        _PVM_(pvm)
  {}
  ...
  gtsam::Vector evaluateError(const gtsam::Pose3& pose_i, const gtsam::Pose3& pose_j,
                              const gtsam::imuBias::ConstantBias& vbias_i, boost::optional<gtsam::Matrix&> H1,
                              boost::optional<gtsam::Matrix&> H2, boost::optional<gtsam::Matrix&> H3) const
  {
    {
